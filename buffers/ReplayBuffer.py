...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed, device = None):
        ...
        self.device = ReplayBuffer.device if device is None else device
    ...
    def sample(self, episode_count = None):
        ...
        weights = torch.from_numpy(np.ones(shape=(self.batch_size, 1), dtype=np.float32)).float().to(self.device)
